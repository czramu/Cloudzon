...
#email.password=CloudRock@135
	...
	public function mail_for_client_buy($email)
	{
			date_default_timezone_set('Etc/UTC');
...
#email.password=CloudRock@135
			...
			//$mail->AddAttachment('MVC\\var\\tmp\\file.zip');
			$mail->addAttachment('MVC\\var\\tmp\\file.zip', 'templet.jpg'); 
			
			$mail->IsHTML(true);                                  // Set email format to HTML
			
			$mail->Subject = 'CloudZon';
			$mail->Body    ="Dear ".$email.",<br><br>Thanks for choosing CloudZon.<br><br>We will revert back you soon.<br><br>Thanks,<br>Team CloudZon";
	...
	public function buy($email,$amount,$transfer,$conn)
	{
		//$sql1="insert into buy_template(name,contact,email) values('$name','$email','$contact')";
		$now=date('Y-m-d H:i:s');
		$sql="insert into buy_template(email,amount,transfer,date) values('$email','$amount','$transfer','$now')";
		$ex=mysqli_query($conn,$sql);
		return $ex;
	}
...
//for admin
	...
	public function adjob($n,$e,$l,$j)
	{
		$a="insert into add_job(title,experience,location,jobdep)values('$n','$e','$l','$j')";
		$f=mysqli_connect("localhost","root","","cz_cloudzon");
		$b=mysqli_query($f,$a);
		return $b;
	}
